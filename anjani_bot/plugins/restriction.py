...
class Restrictions(plugin.Plugin):
    ...
    async def kick_member(self, message):
        ...
        if user is None:
            return await message.reply_text(
                await self.bot.text(chat_id, "no-kick-user")
            )
        try:
            if await user_ban_protected(self.bot, chat_id, user):
                return await message.reply_text(
                    await self.bot.text(chat_id, "admin-kick")
                )
        except UserNotParticipant:
            return await message.reply_text(
                await self.bot.text(chat_id, "err-not-participant")
            )
    ...
    async def ban_member(self, message):
        ...
        try:
            if await user_ban_protected(self.bot, chat_id, user):
                return await message.reply_text(
                    await self.bot.text(chat_id, "admin-ban")
                )
        except UserNotParticipant:
            return await message.reply_text(
                await self.bot.text(chat_id, "err-not-participant")
    ...
    async def unban_member(self, message):
        ...
        if user is None:
            return await message.reply_text(
                await self.bot.text(message.chat.id, "unban-no-user")
            )
        try:
            await message.chat.unban_member(user)
