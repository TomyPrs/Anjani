...
def command(commands: Union[str, List[str]], case_sensitive: bool = False):
    ...
    return create(
        func, "CustomCommandFilter", commands=commands, case_sensitive=case_sensitive
    )
...
async def check_perm(flt, client, message: Message) -> bool:
    ...
    if flt.can_change_info and not (bot.can_change_info and user.can_change_info):
        ...
        and (user.can_restrict_members or user in client.__bot__.staff_id)
    ):
        perm = False
        ...
        and (user.can_promote_members or user in client.__bot__.staff_id)
    ):
        perm = False
